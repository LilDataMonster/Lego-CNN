...
from samples.brixilated_lego.config import LegoConfig


class ModelLoader:
    def __init__(self, labels):
        LEGO_MODEL_PATH = os.path.join(MASK_RCNN_DIR, "mask_rcnn_lego.h5")
        if LEGO_MODEL_PATH is None:
            raise OSError('Model path env not found in the system.')

        class InferenceConfig(LegoConfig):
            GPU_COUNT = 1
            IMAGES_PER_GPU = 1
        ...
        self.model.load_weights(LEGO_MODEL_PATH, by_name=True)
        self.labels = labels
